...
![img/2sem/image.png](img/2sem/image.png)
...
Вот отформатированная версия DEMO-раздела с понятной структурой через заголовки ###:
...
### 2. Магические методы `__repr__` и `__str__`
...
### 6. Сравнение объектов через `__eq__`
...
### 11. Ошибки валидации при создании (через `Validator`)

| Проверка | Ошибка |
|----------|--------|
| Тип названия | название должно быть строкой |
| Пустое название | название не может быть пустым |
| Слишком длинное название | название не может быть длиннее 20 символов |
| Отрицательная цена | цена не может быть отрицательной |
| Цена слишком большая | цена не может быть больше 1000000 |
| Цена не число | цена должна быть числом |
| Отрицательное количество | количество не может быть отрицательным |
| Количество не целое число | количество должно быть целым числом |
| Некорректный статус | статус должен быть одним из: ['in_stock', 'out_of_stock', 'discontinued', 'preorder'] |
| Статус не строка | статус должен быть строкой |
