...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    try:
        ...
        raise ValueError("вообще пустой файл или некорректный JSON")
    ...
    # Проверка количества записей (исключая заголовок)
    with p_csv.open('r', encoding='utf-8') as f:
        reader = csv.reader(f)
        csv_count = len(list(reader)) - 1  # вычитаем заголовок
...
def csv_to_json(csv_path: str, json_path: str) -> None:
    ...
    try:
        ...
        try:
            with p_csv.open('r', encoding='cp1251') as f:
                reader = csv.DictReader(f)
                if not reader.fieldnames:
                    raise ValueError('нет заголовка или пустой')
                data = [row for row in reader]
            ...
            try:
                with p_csv.open('r', encoding='latin-1') as f:
                    reader = csv.DictReader(f)
                    if not reader.fieldnames:
                        raise ValueError('нет заголовка или пустой')
                    data = [row for row in reader]
            except UnicodeDecodeError as e:
                raise UnicodeDecodeError(f"Не удалось декодировать файл. Поддерживаемые кодировки: utf-8, cp1251, latin-1. Ошибка: {e}")
